...
//! should sense, converted into how big the world needs to be for that to hold.
...
impl Probe {
    ...
    pub fn new(params: &Params) -> Probe {
        let trial_box_len = params.radius * TRIAL_BOX_RADII;
        Probe { trial_box_len, measured_coordination: measure_coordination(params, trial_box_len), dimensions: params.dimensions }
    }
...
fn measure_coordination(params: &Params, trial_box_len: f32) -> f32 {
    let probe_count = params.particle_count.min(PROBE_PARTICLES);
    let density_ratio = params.particle_count.max(1) as f64 / probe_count as f64;
    let mut probe_params = params.clone();
    probe_params.particle_count = probe_count; probe_params.box_len = trial_box_len; probe_params.seed = CALIBRATION_SEED;
    let substrate = Substrate::build(&probe_params);
    let nominal = [Shell { amp: 1.0, peak: params.radius, width: params.radius * 0.5 }];
