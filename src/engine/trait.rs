...
//! and wraps at the `0.0`/`1.0` seam instead of clamping.
...
pub fn seed_traits(substrate: &mut Substrate, logits: &[f32], seed: u64) {
    ...
    let exact: Vec<f32> = weights
        .iter().map(|weight| weight / total * substrate.len() as f32).collect();
    ...
    for &bin in order.iter().cycle()
        .take(substrate.len() - counts.iter().sum::<usize>())
    { counts[bin] += 1; }
...
pub fn membership(trait_value: f32, anchors: usize) -> Membership {
    let scaled = trait_value.clamp(0.0, 1.0) * anchors as f32;
    let lower = (scaled.floor() as usize) % anchors;
    let upper = (lower + 1) % anchors;
