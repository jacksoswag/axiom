...
const WIDTH: usize = 1;
pub const METRIC: Metric = &Spec { motion: true, sides: &[SIDE], ..Spec::of("turnover", WIDTH, measure) };
...
pub fn measure(base: &Blocks, _: &[f32]) -> Vec<f32> {
    ...
    let moved = base.motion.as_ref().and_then(|was| was.field.as_ref()).map_or(0.0, |before| {
