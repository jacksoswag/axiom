//! Every knob a run can turn, in one place. One Tuning value fully determines a campaign: the
//! fixed world, discovery search, the viability gates, the repair protocol, which expensive
//! tiers run, and how the scheduling model is fit. Nothing else in tuner holds a tunable
//! constant. Deliberately absent: descriptor geometry (RDF_BINS, HETEROGENEITY_SIDES, LAGS,
//! BARS, SAMPLES, MOBILITY_CEILING) and NEIGHBORS, which define what a descriptor axis means
//! and change by bumping DESCRIPTOR_VERSION, never per run. Anything here that changes what
//! counts as evidence feeds Tuning::digest, carried by ExperimentIdentity, so two campaigns
//! tuned differently cannot merge their ledgers by accident.
...
pub struct Mutation {
    pub iso: f32,
    pub line: f32,
    pub stalled_spread: f32, // spread multiplier applied while the search is stalled
}
impl Default for Mutation {
    fn default() -> Self { Mutation { iso: 0.01, line: 0.2, stalled_spread: 1.8 } }
}

/// Share of each generation spent on novelty parents and goal-directed expeditions. Whatever
/// the two do not claim goes to random births, a random escape route at every batch size.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct Lanes {
    pub novelty: f32,
    pub expedition: f32,
...
impl Default for Lanes {
    fn default() -> Self { Lanes { novelty: 0.7, expedition: 0.2, stalled_novelty: 0.5, stalled_expedition: 0.3 } }
}
impl Lanes {
    ...
    pub fn counts(&self, batch: usize, stalled: bool) -> LaneCounts {
        let (novelty_share, expedition_share) =
            if stalled { (self.stalled_novelty, self.stalled_expedition) } else { (self.novelty, self.expedition) };
        let novelty = share(batch, novelty_share);
        let expedition = share(batch, expedition_share).min(batch - novelty);
        LaneCounts { novelty, expedition, random: batch - novelty - expedition }
    }
}
...
pub struct LaneCounts { pub novelty: usize, pub expedition: usize, pub random: usize }

/// Binary admission and promotion thresholds. These never combine into a scalar fitness; each
/// is a named clause a world either clears or fails.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct Gates {
    ...
    pub dispersed_structure: f32, // structure at or below which a rollout is watched for sustained dispersion
    pub collapsed_structure: f32, // structure at or above which a rollout is watched for sustained collapse
    pub sustained_windows: usize, // consecutive watchdog windows before an early stop
}
impl Default for Gates {
    fn default() -> Self {
        Gates { structure_floor: 0.05, structure_ceiling: 1.0, variance_floor: 1e-4, robustness_floor: 0.66,
            heterogeneity_floor: 0.01, connected_fraction_floor: 0.75, mobility_floor: 0.001,
            dispersed_structure: 0.01, collapsed_structure: 1.25, sustained_windows: 3 }
    }
}
...
pub struct Repair {
    pub probes: usize,
    pub shake: f32, // displacement applied to the damaged region, in kernel cutoffs
    pub recovery: f32, // fraction of the original damage a world must close to count as recovered
    pub min_effective_damage: f32, // damage below this proves nothing, the probe is discarded
}
impl Default for Repair {
    fn default() -> Self { Repair { probes: 3, shake: 0.5, recovery: 0.5, min_effective_damage: 0.01 } }
}
...
pub struct Discovery {
    ...
    pub lanes: Lanes,
    pub curated_parent_indices: Vec<usize>, // browser pins: expedition parents only, pruned like the rest
}
impl Default for Discovery {
    fn default() -> Self {
        Discovery { generations: 30, steps: 1500, batch: 32, initial: 64, capacity: 2000, seed: 7,
            promotion_budget: 0, stall_window: 8, mutation: Mutation::default(), lanes: Lanes::default(),
...
impl Default for Learning {
    fn default() -> Self {
        Learning { minimum_labeled_rows: 30, model_seed: 0xA71_0A0,
            quotas: ContinuationQuotas { high_survival: 4, high_uncertainty: 3, uniform_neighborhood: 3 },
...
impl Tuning {
    ...
    pub fn digest(&self) -> u64 {
        ...
        let Gates { structure_floor, structure_ceiling, variance_floor, robustness_floor, heterogeneity_floor,
            connected_fraction_floor, mobility_floor, dispersed_structure, collapsed_structure, sustained_windows } = self.gates;
        for value in [structure_floor, structure_ceiling, variance_floor, robustness_floor, heterogeneity_floor,
            connected_fraction_floor, mobility_floor, dispersed_structure, collapsed_structure]
        { hash.float(value); }
        hash.bytes((sustained_windows as u64).to_le_bytes());
        let Repair { probes, shake, recovery, min_effective_damage } = self.repair;
        hash.bytes((probes as u64).to_le_bytes());
        for value in [shake, recovery, min_effective_damage] { hash.float(value); }
        let Mutation { iso, line, stalled_spread } = self.discovery.mutation;
        let Lanes { novelty, expedition, stalled_novelty, stalled_expedition } = self.discovery.lanes;
        for value in [iso, line, stalled_spread, novelty, expedition, stalled_novelty, stalled_expedition] { hash.float(value); }
        hash.bytes((self.discovery.stall_window as u64).to_le_bytes());
        hash.finish()
    }
}

/// One named setting, addressable as a key=value pair. The same table drives CLI parsing,
/// generated help, and the archive header, so a knob cannot exist without documentation or
/// drift out of sync with its help text.
pub struct Knob {
    pub key: &'static str,
    pub help: &'static str,
...
impl Knob {
    pub fn set(&self, tuning: &mut Tuning, value: &str) -> Result<(), String> { (self.setter)(tuning, value) }
    pub fn show(&self, tuning: &Tuning) -> String { (self.shower)(tuning) }
}
    ...
    ($key:literal, $help:literal, $($field:ident).+, $ty:ty) => {
        Knob {
            ...
            setter: |cfg: &mut Tuning, raw: &str| {
                cfg.$($field).+ = raw.parse::<$ty>().map_err(|_| format!("{} expects {}", $key, stringify!($ty)))?;
                Ok(())
            },
...
macro_rules! tier_knob {
    ($key:literal, $help:literal, $field:ident) => {
        Knob {
            key: $key, help: $help,
            setter: |cfg: &mut Tuning, raw: &str| {
                let steps: usize = raw.parse().map_err(|_| format!("{} expects a step count, or 0 to disable", $key))?;
...
pub const KNOBS: &[Knob] = &[
    // world
    knob!("particles", "particles per rollout", world.particle_count, usize),
    knob!("anchors", "continuous-trait anchors", world.anchor_count, usize),
...
pub fn knob(key: &str) -> Option<&'static Knob> { KNOBS.iter().find(|knob| knob.key == key) }

/// Apply one key=value setting.
pub fn apply(tuning: &mut Tuning, key: &str, value: &str) -> Result<(), String> {
    knob(key).ok_or_else(|| format!("unknown option {key:?}"))?.set(tuning, value)
}
...
pub fn help(tuning: &Tuning) -> String {
    ...
    KNOBS.iter().map(|knob| format!("  {:width$} = {:<10} {}\n", knob.key, knob.show(tuning), knob.help)).collect()
}
